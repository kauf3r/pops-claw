...
<task type="auto">
  ...
  <action>
Re-auth both Gmail accounts to remove excess `gmail.settings.basic` and `gmail.settings.sharing` scopes. Per user decision: one-shot re-auth, both accounts in same session.

**IMPORTANT: Locked decision vs gog limitation**
User locked decision specifies `calendar.readonly`. However, research (28-RESEARCH.md) found that gog does NOT support per-service readonly — `--services calendar` always grants full calendar read+write scope. There is no `--services calendar.readonly` flag.

Before proceeding with re-auth, surface this to the user in the checkpoint (Task 2). If the user approves full calendar scope as acceptable (gog's only option), proceed with `--services gmail,calendar`. If the user requires strict `calendar.readonly`, we'd need a different OAuth tool — that's out of scope for this phase.
...
- `gmail.modify` already subsumes `gmail.readonly` and `gmail.send` per Google scope hierarchy.
...
```
Record the result — this informs the checkpoint discussion in Task 2 about whether full calendar scope is justified or just gog's limitation.
...
**Step 3: Open interactive SSH session and re-auth personal account (theandykaufman@gmail.com)**
This is interactive -- requires TTY for OAuth flow. **Steps 3-7 all run within this same interactive SSH session.** Do NOT exit between steps.
```bash
ssh -t -i ~/.ssh/clawdbot-key.pem ubuntu@100.72.143.9
```
Then on EC2 (all remaining commands in this session):
```bash
...
**Step 4: Verify personal account** (still in the same SSH session)
```bash
gog auth list
...
**Step 5: Re-auth AirSpace account (Kaufman@AirSpaceIntegration.com)** (still in the same SSH session)
```bash
...
**Step 6: Verify AirSpace account** (still in the same SSH session)
```bash
gog auth list
...
**Step 7: Smoke test cron jobs that use gog** (still in the same SSH session)
...
**Step 8: Restart gateway and exit SSH session**
```bash
...
**Scope conflict requiring sign-off:** Your locked decision specified `calendar.readonly`, but gog does not support per-service readonly scopes — `--services calendar` always grants full calendar read+write. This means the re-auth used full calendar scope instead of `calendar.readonly`. Please confirm this is acceptable (gog's only option short of switching OAuth tools, which is out of scope).

How to verify:
1. Check the summary output from Task 1 -- confirm both accounts show trimmed scopes (excess `gmail.settings.basic` and `gmail.settings.sharing` removed)
2. Confirm you accept full calendar scope as a gog limitation (your locked decision was `calendar.readonly`, but gog can't provide that granularity)
3. If Task 1 encountered issues with the AirSpace Workspace account (admin re-approval needed), confirm whether you completed the Workspace admin approval
4. Confirm you're satisfied with the scope set before we proceed to documentation

Resume signal: Type "approved" to proceed to gateway documentation, or describe any issues with the OAuth re-auth or calendar scope.
  </action>
