# 03-03 Cron Job Evidence

Captures cron job configurations created on EC2 (100.72.143.9) for plan 03-03.

## Evening Recap (Task 1)

- **ID:** 1914a2ba-5d4d-49c4-a884-a7e7b7aee17b
- **Name:** evening-recap
- **Schedule:** `0 19 * * *` (America/Los_Angeles) = 7 PM PT daily
- **Session:** main
- **Model:** sonnet
- **Wake:** now
- **Timeout:** 90s
- **Target DM:** D0AARQR0Y4V
- **Created:** 2026-02-08

Content: Summarizes today's activity, previews tomorrow's calendar via `gog calendar list`, lists top open items.

## Weekly Review (Task 2)

- **ID:** 058f0007-935b-4399-aae1-28f6735f09ce
- **Name:** weekly-review
- **Schedule:** `0 8 * * 0` (America/Los_Angeles) = Sunday 8 AM PT
- **Session:** main
- **Model:** sonnet
- **Wake:** now
- **Timeout:** 120s
- **Target DM:** D0AARQR0Y4V
- **Created:** 2026-02-08

Content: 7-day health trends from health.db (sleep, readiness, HRV, resting HR, activity), week-in-review grouped by theme, upcoming week calendar via `gog calendar list`, actionable recommendations.
